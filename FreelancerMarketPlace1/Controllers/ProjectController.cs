...
namespace FreelancerMarketPlace1.Controllers
{
	public class ProjectController : Controller
	{
		...
		public async Task< IActionResult> Index(int id)
		{
            ...
            var ProjectData = _context.Projects.Where(x => x.ProjectID == 1).FirstOrDefault(); // Linq sorgusu ToList : listele 
			model.ProjectAll = ProjectData;
		...
		public async Task<IActionResult> ApplyProject(int projectId)
		{
            ...
            return RedirectToAction("ProjectDetail", new { id = 1 });
		}
		[HttpGet]
		public IActionResult ProjectList(string skillName)
		{
			...
			if(skillName != null)
			{
				values=_context.Projects.Where(x=>x.SkillCalledSt==skillName).ToList();
			}
		...
		{
			var projectCompleted = _context.Projects.Count(x => x.ProjectState == "Tamamlandı");
			ViewBag.projectCompleted = projectCompleted.ToString();
			var projectPosted = _context.Projects.Count(x => x.ProjectState == "Yayında");
			ViewBag.projectPosted = projectPosted.ToString();
			var projectOngoing = _context.Projects.Count(x => x.ProjectState == "İş Alındı");
